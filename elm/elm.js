...
Elm.Main.make = function (_elm) {
   ...
   var quicksort = function (list) {
      return function () {
         switch (list.ctor)
         {case "::": return function () {
                 var higher = A2($List.filter,
                 function (n) {
                    return _U.cmp(n,list._0) > 0;
                 },
                 list._1);
                 var lower = A2($List.filter,
                 function (n) {
                    return _U.cmp(n,list._0) < 1;
                 },
                 list._1);
   ...
   var length = function (list) {
      return function () {
         switch (list.ctor)
         {case "::":
            return 1 + length(list._1);
            case "[]": return 0;}
         _U.badCase($moduleName,
         "between lines 9 and 11");
      }();
   };
   var main = $Graphics$Element.show({ctor: "_Tuple3"
                                     ,_0: length(_L.range(1,9))
                                     ,_1: A2(zip,
                                     _L.fromArray(["Tom"
                                                  ,"Sue"
                                                  ,"Bob"]),
                                     _L.fromArray([45,31,26]))
                                     ,_2: quicksort(_L.fromArray([5
                                                                 ,6
                                                                 ,3
                                                                 ,5
                                                                 ,34
